...
# 残差连接部分
class DeforConvolutionBlock(nn.Module):
    def __init__(self, num_filters):
    ...
    def forward(self, x):
        residual = x
        out = self.conv1(x)
        out = self.offset12(out)
        out = self.conv2(out)
        out = out + residual
        return out
class DeforResblock(nn.Module):
    def __init__(self, inc, num_filters, num_blocks):
        super(DeforResblock, self).__init__()
        self.conv1 = Conv2D_BN_Leaky(inc, num_filters, kernel=3, strid=2)
        self.res_body = self.makelayers(num_filters, num_blocks)
...
class yolo_body(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv = Conv2D_BN_Leaky(3, 32)
        self.resblck1 = DeforResblock(32, 64, 1)
        self.resblck2 = DeforResblock(64, 128, 2)
        self.resblck3 = DeforResblock(128, 256, 4)
        self.resblck4 = DeforResblock(256, 512, 8)
        self.resblck5 = DeforResblock(512, 1024, 8)
        self.resblck6 = DeforResblock(1024, 2048, 4)
        self.yolo_block1 = yolo_block(2048, 1024, len(config.dataSet['anchors_mask'][0]) * (num_classes + 6))
        self.yolo_block2 = yolo_block(1536, 512, len(config.dataSet['anchors_mask'][1]) * (num_classes + 6))
        # self.yolo_block1 = yolo_block(1024, 512, len(config.dataSet['anchors_mask'][0]) * (num_classes + 6))
        self.yolo_block3 = yolo_block(768, 256, len(config.dataSet['anchors_mask'][1])* (num_classes + 6))
        self.seLayer1=SELayer(64)
        self.seLayer2 = SELayer(128)
        self.seLayer3 = SELayer(256)
        self.seLayer4 = SELayer(512)
        self.seLayer5 = SELayer(1024)
        self.seLayer6 = SELayer(2048)
    ...
    def forward(self, x):
        ...
        out1 = self.seLayer3(out1)
        out2 = self.resblck4(out1)
        out2 = self.seLayer4(out2)
        out3 = self.resblck5(out2)
        out3 = self.seLayer5(out3)
        out4 = self.resblck6(out3)
        out4=self.seLayer6(out4)
        out, y1 = self.yolo_block1(out4)
