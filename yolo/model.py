...
def yolo_loss(out_put, y_true, conf_false_mask, num_classes, anchors, input_shape, ratio, CUDA, loss_function='None',
              print_loss=True):
    m = len(out_put)
    loss = 0
    flag = 0
    for i in range(m):  # i:layer
        ...
        if loss_function == 'focal_loss':
            ...
            confidence_loss = ((1 - conf_false_mask_) * F.binary_cross_entropy_with_logits(X[..., 5, :, :],
                                                                                           Y[..., 5, :, :],
                                                                                           reduction='none') + Y[..., 5,
                                                                                                               :,
                                                                                                               :] * F.binary_cross_entropy_with_logits(
                X[..., 5, :, :], Y[..., 5, :, :], reduction='none')) * 0.01
            # confidence_loss = F.binary_cross_entropy_with_logits(X[...,5,:,:],Y[...,5,:,:],reduction='none')*0.0001
            cla_loss = F.binary_cross_entropy_with_logits(X[..., 6:, :, :], Y[..., 6:, :, :],
                                                          reduction='none') * object_mask
        ...
        if print_loss:
            print('loss:{:<10.3f}'.format(scal_loss), 'xy_loss:{:<10.3f}'.format(xy_loss.sum()),
                  'wh_loss:{:<10.3f}'.format(wh_loss.sum()), 'angle_loss:{:<10.3f}'.format(angle_loss.sum()),
                  'confidence_loss:{:<10.3f}'.format(confidence_loss.sum()),
